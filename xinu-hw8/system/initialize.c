...
struct sement semtab[NSEM];     /* List of semaphores.                             */
memblk freelist;				/* List of free memory blocks					*/
...
spinlock_t memlock;				/* spinlock for memory list			   */
...
static int sysinit(void)
{
	...
	/* Initialize free memory list and lock */
	memlock = lock_create();
	memheap = roundmb(memheap);
	platform.maxaddr = truncmb(platform.maxaddr);
	freelist.next = pmblock = (memblk *) memheap;
	freelist.length = (uint)(platform.maxaddr - memheap);
	pmblock->next = NULL;
